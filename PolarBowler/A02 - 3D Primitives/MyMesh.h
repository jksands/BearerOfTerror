...
#pragma region 3D Primitives
	...
	*/
	...
	/*
	USAGE: Generates a circle
	ARGUMENTS:
	-	float a_fRadius -> Radius of the circle
	-	int a_nSubdivisions -> Amount of subdivisions of the circle
	-	vector3 centerPos -> center of the circle
	-	bool flipped -> determines which way to add the tri points
	-	vector3 a_v3Color -> Color of the mesh
	OUTPUT: ---
	*/
	std::vector<vector3> GenerateCircle(float a_fRadius, int a_nSubdivisions, vector3 centerPos, bool flipped, vector3 a_v3Color = C_WHITE);

	std::vector<vector3> GenerateParametricCircle(float a_fRadius, int a_nSubdivisions, vector3 centerPos, float angleOfRotation, vector3 a_v3Color = C_WHITE);
	/*
